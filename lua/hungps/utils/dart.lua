...
M.line_length_sources = {
  { name = "analysis_options", path = "analysis_options.yaml", pattern = "page_width: (%d-)[\n%[$]" },
  { name = "vscode", path = ".vscode/settings.json", pattern = '"dart.lineLength": (%d-),' },
  { name = "editorconfig", path = ".editorconfig", pattern = "%[%*%.dart%].-max_line_length%s-=%s-(%d-)[\n%[$]" },
}
...
M.get_line_length = function()
  ...
  for _, source in ipairs(M.line_length_sources) do
    local match = file_utils.match(cwd .. source.path, source.pattern)

    if match then
      print("Using line length " .. match .. " from " .. source.name)
